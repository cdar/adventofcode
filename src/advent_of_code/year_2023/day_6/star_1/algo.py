"""
you get a sheet of paper (your puzzle input) that lists the time allowed for each race
and also the best distance ever recorded in that race. To guarantee you win the grand
...
This document describes three races:
    (ms - milliseconds, mm - millimeters)
...
So, because the first race lasts 7 milliseconds, you only have a few options:
    ...
    Hold the button for 5 milliseconds, causing the boat to travel a total of 10 mm.
    Hold the button for 6 milliseconds, causing the boat to travel a total of 6 mm.
...
def parse(text: str) -> list[list[int]]:
    return [list(map(int, line.split()[1:])) for line in text.splitlines()]


def algo(times: list[int], distance: list[int]) -> int:
    # """
    # t - time
    # d - distance
    # tt - total time
    ...
    def x1(tt, d):
        return (-tt - pow(pow(tt, 2) - 4 * d, 1 / 2)) / (-2)
    ...
    for total_time, best_distance in zip(times, distance):
        root_of_function = sorted(
            (x1(total_time, best_distance), x2(total_time, best_distance))
        )
        result = [
            math.floor(root_of_function[0] + 1),
            math.ceil(root_of_function[1] - 1),
        ]
...
def main():
    ...
    result = algo(*parse(text))
    print(result)
    print("expected: 138915")
