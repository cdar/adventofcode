...
def parse(data: str) -> Data:
    ...
    lines_iter = iter(lines[2:])
    mapping = {}

    for line in lines_iter:
        assert "map:" in line
        mapping_names = line.split()[0].split("-")
        mapping_names = (mapping_names[0], mapping_names[2])
        ...
        for line in lines_iter:
            if line:
                ...
                mapping_numbers.append(numbers)
            else:
                break

        mapping_numbers.sort(key=lambda el: el[0])
        mapping[mapping_names] = mapping_numbers

    return Data(seeds, mapping)


def get_highway(mapping_name: Iterable[tuple[str, str]]) -> list[str]:
    mapping = {i[1]: i[0] for i in mapping_name}
    current = END_NAME
    result = [END_NAME]
